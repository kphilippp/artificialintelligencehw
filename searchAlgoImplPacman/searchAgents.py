...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # we have the walls, the starting pos, the corners, and the number of nodes expanded
        # we need to better define the problem so that we can actually run the algo on it.
        # the start state is given
        # the the goal state is where all corners have been eaten
...
def cornersHeuristic(state, problem):
    ...
    # use the manhattan distance formula but lets greedily take all unviisted node distances
    # this is admissable because the true cost must be at least as high as the farhest unvisited node
    ...
    for index, corner in enumerate(corners):
        if not visitedCorners[index]:
            unvisitedCornerList.append(corner)
    if not unvisitedCornerList:
        return 0
